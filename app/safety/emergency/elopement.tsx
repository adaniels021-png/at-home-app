...
export default function ElopementSearchScreen() {
  ...
  return <Shell onBack={handleBack} scroll gestureEnabled>
    ...
    <View style={styles.summaryCard}><Summary label={latestSighting ? "LATEST SIGHTING" : "LAST SEEN"} value={latestSighting?.placeLabel || incident?.lastSeenPlaceLabel || 'Not added yet'} detail={latestSighting?.sightingTime ? formatClock(latestSighting.sightingTime) : incident?.lastSeenTime ? formatClock(incident.lastSeenTime) : 'Using incident start time'} onEdit={latestSighting ? undefined : () => { setSnapshotMode('edit'); setStage('location'); }} /><View style={styles.divider} /><Summary label="WEARING" value={incident?.currentClothing || 'Not added yet'} onEdit={() => { setSnapshotMode('edit'); setStage('clothing'); }} /></View>
